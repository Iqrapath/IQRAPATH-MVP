      ...
      <Card className="mb-8 shadow-sm">
        <CardContent className="p-6">
          ...
          <div className="flex gap-3 items-center">
            ...
            {requestStatus !== 'live_video' && videoStatus === 'scheduled' && (
              <Button variant="outline" onClick={startLive}>Start Live Verification</Button>
            )}
            {requestStatus === 'live_video' && videoStatus === 'scheduled' && (
              <>
                <div className="text-blue-600 font-medium mb-2">🔄 Live verification in progress...</div>
                <Button className="bg-green-600 hover:bg-green-700" onClick={() => openCompletionModal('passed')}>Mark as Passed</Button>
                <Button variant="destructive" onClick={() => openCompletionModal('failed')}>Mark as Failed</Button>
              </>
            )}
            {videoStatus === 'passed' && (
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            >
              Mark as {pendingOutcome === 'passed' ? 'Passed' : 'Failed'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
